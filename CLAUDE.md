...
- The project file and `data/` are Lorne's, but additive/reversible disk changes that follow
  from what was asked are fine to do directly: new files, `.new.tif`-style outputs, edits to
  manifest/docs. Destructive actions stay Lorne's: overwriting or deleting existing data,
  saving the project, committing. In-memory QGIS changes are fine when asked in that message;
...
- Scripts for repeated/heavy tasks; run them yourself when their effects are additive or
  reversible per the rule above. PyQGIS scripts that must run inside QGIS go to Lorne (console).
